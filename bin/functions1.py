...
def mng_tab_select(arr,line,offset,select,ch_T_SP,remove=False):
    ...
    # Add indentation or remove it
    if not remove: p1=[tab+x for x in p1]
    else: p1 = [x[len(tab):] if x.startswith(tab) else x for x in p1] 
